...
function renderItems(items, rows, cols, size, padding) {
  ...
  for(let i = 0; i < rows * cols; i++) {
    ...
    const color = items[i].active ? '#FFFFFF' : '#000000';
...
class Grid extends Component {
  render() {
    const { grid, countRows, countCols, pixelSize, padding } = this.props;
    ...
    const items = renderItems(grid, countRows, countCols, pixelSize, padding);
    return(
      <svg
